...
export default function (slug, model, model_plurial, sort_items = null) {
  return {
    ...
    getters: {
      getItems: state =>
        JSON.parse(JSON.stringify(state[model_plurial] || [])),
      getItem: state => id =>
        JSON.parse(
          JSON.stringify(
            (state[model_plurial] || []).find(
              item => item.id === id
            ) || {}
          )
        ),
      getSelectedItem: state =>
        JSON.parse(JSON.stringify(state[model] || {}))
    },
    ...
    actions: {
      fetchItems: ({ commit }, params = null) => {
        return apiRequest(
          `${slug}/index`,
          "get",
          payload => commit("SET_ITEMS", payload),
          params
        );
      },
      fetchItem: ({ commit }, id) => {
        return apiRequest(`${slug}/show/${id}`, "get", payload =>
      ...
      forceRemoveItems: ({ commit }, ids) => {
        return apiRequest(
          `${slug}/force-destroy`,
          "put",
          payload => commit("REMOVE_ITEMS", ids),
          { ids }
        );
      },
      duplicateItem: ({ commit }, ids) => {
        return apiRequest(
          `${slug}/duplicate`,
          "put",
          payload => commit("ADD_OR_UPDATE_ITEMS", payload),
