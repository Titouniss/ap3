  ...
  <div style="min-width: 200px">
    ...
    <div class="filesContainer">
      ...
      >
        ...
        />
        <label class="fileContainer py-2 px-3 w-1/2" @click="isUrlMode = true">
...
export default {
  ...
  computed: {
    ...
    displayItems() {
      return this.items.filter((item) => this.hiddenItems.indexOf(item) === -1);
    },
  },
  methods: {
    openUrl(url) {
    ...
    onFileAdd(e) {
      ...
      if (files.length > 0) {
        ...
        // Validate file type
        let error =
          ["application/pdf", "image/jpeg", "image/png"].indexOf(file.type) ===
          -1
    ...
    uploadFile(item, is_file = true) {
      const action = `documentManagement/${is_file ? "upload" : "addItem"}`;
