    ...
    <div class="flex">
        ...
        >
            <vx-auto-suggest
                ...
                :data="filteredNavbarSearchAndPinList"
                search_key="title"
                background-overlay
                class="w-full"
...
export default {
    ...
    computed: {
        filteredNavbarSearchAndPinList() {
            const userHasPermissionTo = this.$store.getters.userHasPermissionTo;

            const clone = JSON.parse(JSON.stringify(navbarSearchAndPinList));
            clone.pages.data = clone.pages.data.filter(
                item =>
                    item.slug === "home" ||
                    userHasPermissionTo(`show ${item.slug}`)
            );
