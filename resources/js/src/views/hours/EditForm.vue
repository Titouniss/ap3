...
export default {
  ...
  computed: {
    ...
    activePrompt: {
      get() {
        if (this.itemId && this.itemId > -1) {
          this.itemLocal = this.$store.getters["hoursManagement/getItem"](
            this.itemId
          );
        }
    ...
    projectsData() {
      return this.$store.getters["projectManagement/getItems"].filter(
  ...
  methods: {
    init() {
      this.itemLocal = this.$store.getters["hoursManagement/getSelectedItem"];
    },
    submitHour() {
      var itemToSave = {};
