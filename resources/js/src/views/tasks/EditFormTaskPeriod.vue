    ...
    >
      <div>
        <form class="edit-task-form" autocomplete="off">
          <div class="vx-row">
            <!-- Left -->
            <span class="vx-col flex-1">Nom de la tâche : {{ this.itemLocal.name }}</span>
          </div><br>
          <div class="vx-row">
          ...
          <div class="vx-row">
            <div class="vx-col flex-1">
              <vs-checkbox
                  v-model="moveAccepted"
                  class="mt-6"
              >
              Déplacer toutes les périodes des tâches dépendantes
              </vs-checkbox>
            </div>
          </div>
          <div class="vx-row">
...
export default {
  ...
  data() {
    const item = JSON.parse(
      JSON.stringify(this.$store.getters["taskManagement/getItem"](this.taskId))
    );
    ...
    submitItem() {
      ...
      if(this.unavailabilities_list != null && !this.moveAccepted){
        for(var i=0; i<this.unavailabilities_list.length; i++){            
            if(this.unavailabilities_list[i].date_end != null && (this.unavailabilities_list[i].date != this.unavailabilities_list[i].date_end)){
              if(this.start>this.unavailabilities_list[i].date && this.end<this.unavailabilities_list[i].date_end){
                erreur=true;
                this.$vs.notify({
                  title: "Erreur",
