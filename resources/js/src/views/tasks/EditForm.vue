<template>
  <div class="p-3 mb-4 mr-4">
    ...
    >
      <div
        :class="
          project_data.status == 'waiting' || project_data.status == 'done'
            ? 'disabled-div'
            : null
        "
      >
        <form autocomplete="off" v-on:submit.prevent>
          <div class="vx-row">
            ...
            >
              ...
              >
                <small class="date-label mb-1" style="display: block">
                  Date
                </small>
                <flat-pickr
              ...
              <div
                ...
                :class="
                  itemLocal.project.status == 'doing' ? 'disabled-div' : ''
                "
                v-if="this.type != 'users' && this.type != 'workarea'"
              >
                <simple-select
                  required
                  header="Compétences"
              ...
              <div
                v-if="
                  type !== 'users' && type !== 'workarea' && !hasAvailableUsers
                "
                class="text-danger text-sm"
              >
                Attention, aucun utilisateur ne possède cette combinaison de
                compétences
              </div>
            ...
            <div class="vx-col flex-5">
              <div class="mb-3" style="flex-direction: column; display: flex">
                <span v-if="itemLocal.project.status !== 'doing'">
                  <add-previous-tasks
                ...
                <div
                  ...
                  "
                >
                  <span v-on:click="showDescription" class="linkTxt">
...
export default {
  ...
  computed: {
    ...
    validateForm() {
      const { name, estimated_time, date, skills, workarea_id, user_id } =
        this.itemLocal;
    ...
    hasAvailableUsers() {
      return (
        ...
        (this.itemLocal.skills.length > 0 && this.usersDataFiltered.length > 0)
      );
    },
    hasAvailableWorkareas() {
      return (
  ...
  methods: {
    ...
    addSupply(value) {
      console.log(this.customSupplies.id);
      let supplies_local = [];
