          ...
          <vs-dropdown vs-trigger-click class="cursor-pointer">
            ...
            >
              <span class="mr-2">
                ...
                -
                {{
                  projectsData.length - currentPage * paginationPageSize > 0
  ...
  computed: {
    ...
    ganttProjectsData() {
      ...
      return this.projectsData
        .filter(
          (p) =>
            p.status === "doing" && p.start_date && moment(p.date).isAfter()
        )
        .map((p) => ({
          id: p.id.toString(),
          name: p.name || "",
  ...
  methods: {
    ...
    getProjectStatusColor(project) {
      if (project.progress === 100) {
        ...
        moment(project.end || project.date).isAfter(moment().add(1, "month"))
      ) {
        return "primary";
      } else if (
        moment(project.end || project.date).isAfter(moment().add(1, "w"))
      ) {
        return "warning";
      } else {
        return "danger";
    ...
    onResize(event) {
      if (this.ganttProjectsData.length > 0) {
        this.gantt = new Gantt("#gantt", this.ganttProjectsData, {
          ...
          // Events
          on_click: (project) => {
            this.$router.push(`/projects/project-view/${project.id}`);
          },
        });
      }
