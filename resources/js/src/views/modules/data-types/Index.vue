          ...
          <form autocomplete="off" :data-vv-scope="item.slug">
            <div class="w-full my-6 flex-row items-center">
              <vs-input
                ...
                v-model="item.source"
                :label-placeholder="`${
                  module.type === 'sql' ? 'Nom de table source' : 'Route source'
                }`"
                :success="
                  item.source.length > 0 && !errors.has('source', item.slug)
                "
            ...
            <vs-table :data="item.module_data_rows" stripe>
              ...
              <template slot-scope="{ data }">
                <vs-tr
                  ...
                  :key="`${item.slug}.${data[rowIndex].field}`"
                >
                  <vs-td :data="data[rowIndex].display_name">
                    {{ data[rowIndex].display_name }}
                    <span v-if="data[rowIndex].required"> * </span>
                  </vs-td>
                  <vs-td :data="data[rowIndex].type">
                    {{
                      `${data[rowIndex].type.charAt(0).toUpperCase()}${data[
                        rowIndex
                      ].type.slice(1)}`
                    }}
                  </vs-td>
                  <vs-td :data="data[rowIndex].source">
                    <vs-input
                      :name="`${data[rowIndex].field}.source`"
                      v-validate="
                        `max:255${
                      ...
                      :success="
                        data[rowIndex].source.length > 0 &&
                        !errors.has(`${data[rowIndex].field}.source`, item.slug)
                      "
                      :danger="
                        errors.has(`${data[rowIndex].field}.source`, item.slug)
                      "
                      :danger-text="`${errors.first(
                  ...
                  <vs-td :data="data[rowIndex].default_value">
                    <vs-input
                      :name="`${data[rowIndex].field}.default_value`"
                      v-validate="
                        `max:255${
                          data[rowIndex].required && !data[rowIndex].source
                            ? '|required'
                            : ''
                        }`
                      "
                      v-model="data[rowIndex].default_value"
                      :success="
                        data[rowIndex].default_value.length > 0 &&
                        !errors.has(
                  ...
                  <vs-td :data="data[rowIndex].details">
                    <vs-input
                      :name="`${data[rowIndex].field}.details`"
                      v-validate="`max:255`"
                      v-model="data[rowIndex].details"
                      :success="
                        data[rowIndex].details.length > 0 &&
                        !errors.has(
                      ...
                      "
                      :danger="
                        errors.has(`${data[rowIndex].field}.details`, item.slug)
                      "
                      :danger-text="
                        errors.first(
                          `${data[rowIndex].field}.details`,
  ...
  computed: {
    ...
    inactiveDataTypes() {
      return this.dataTypes.filter(
        (dt) => !this.activeDataTypes.map((t) => t.data_type_id).includes(dt.id)
      );
    },
  },
  methods: {
      ...
      const dataType = {
        ...
        module_data_rows: this.currentDataType.data_rows.map((dr) => {
          ...
          return {
            ...
            default_value: (mDataRow && mDataRow.default_value) || "",
    ...
    this.$store
      ...
      .then((response) => {
        this.module.module_data_types.forEach((mdt) => {
          const dt = this.dataTypes.find((dt) => dt.id === mdt.data_type_id);
          if (dt) {
