    ...
    <div class="vx-card no-scroll-content" style="border: 1px solid #c1c1c1;">
      <vs-row vs-type="flex" vs-justify="space-around">
        <vs-col
          ...
          vs-w="4"
          class="itemSelector"
          :class="schedulList == 'ilots' ? 'active' : 'not'"
          @click="changeList('ilots')"
        >
          <div class="col">
            <a v-on:click="changeList('ilots')" class="text-dark cursor-pointer">Plannings par îlot</a>
        ...
        <vs-col
          ...
          :class="schedulList == 'users' ? 'active' : 'not'"
        ...
        <vs-col
          ...
          vs-w="4"
          class="itemSelector"
          :class="schedulList == 'projects' ? 'active' : 'not'"
        ...
        >
          ...
          >
            ...
            <div
              class="mt-2"
              style="font-size: 1.1em; color: #000; font-weight: 500"
            >{{user.firstname + ' ' + user.lastname.toUpperCase()}}</div>
          </div>
          <h4 v-if="!usersData" color="red">Aucun utilisateur</h4>
        </div>
        <!-- project list-->
        <div
          v-if="schedulList === 'projects'"
          style="display: flex; flex-direction: row; flex-wrap: wrap; justify-content: center"
        >
          <div
            v-for="project in projectsData"
            v-bind:key="project.id"
            class="card-task-project p-2 m-3"
            @click="goDetail(project.id, 'projects')"
          >
            <div
              class="mt-3 projectStatus"
              :class="project.status"
            >{{ project.status == 'todo' ? 'A compléter' : (project.status == 'doing' ? 'En cours' : null) }}</div>
            <feather-icon
              icon="ActivityIcon"
              svgClasses="h-8 w-8"
            ...
            <div
              class="mb-2"
              style="font-size: 1em; color: #000; font-weight: 300; font-style: italic;"
            >{{ momentTransform(project.created_at) }}</div>
          </div>
