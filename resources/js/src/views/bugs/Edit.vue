...
<template>
  <div id="page-bug-edit">
    ...
    >
      ...
      <span>
        <span>voir</span>
        <router-link :to="{ name: 'bugs' }" class="text-inherit underline"
          >Tout les bugs</router-link
        >
      </span>
    </vs-alert>
    ...
    <vx-card v-if="bug_data">
      <div slot="no-body" class="tabs-container px-6 pt-6">
        ...
        <vs-row vs-justify="left" vs-type="flex" vs-w="12" class="px-6">
          <h2>Bug n° {{bug_data.id}} remonté par <span style="font-weight: 900;">{{bug_data.created_by.lastname + " " + bug_data.created_by.firstname}}</span>, le {{ dateFormatter(bug_data.created_at) }}</h2>
        </vs-row>
        <vs-row vs-justify="left" vs-type="flex" vs-w="12" class="px-6">
           <vs-col vs-w="6" vs-xs="12" class="mt-6" v-if="bug_data && bug_data.company">
             <p> Société : {{bug_data.company.name }} </p>
           </vs-col>
           <vs-col vs-w="6" vs-xs="12" class="mt-6" v-if="bug_data && bug_data.role">
...
export default {
  ...
  data() {
    return {
      ...
      bug_not_found: false,modulesOption: [
